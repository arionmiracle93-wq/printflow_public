...
export function QuickStatusPopup({
  ...
  const modal = open ? (
    ...
    >
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-white/10 bg-white shadow-[0_24px_80px_rgba(0,0,0,.35)] dark:bg-[#101e29]">
        <div className="flex items-center justify-between gap-2 border-b border-teal-100 bg-[#07384f] px-4 py-3 text-white dark:border-teal-900/40">
          <div className="min-w-0">
            <p className="text-[11px] font-bold text-cyan-100/80">{orderCode}</p>
            <p className="truncate text-sm font-extrabold">{orderTitle}</p>
          </div>
          <button
            type="button"
        ...
        <div className="max-h-[70vh] space-y-4 overflow-y-auto p-4">
          <div>
            <p className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">Status Pekerjaan</p>
            <div className="grid grid-cols-3 gap-1.5">
              {STATUSES.map((s) => (
                <button
                  ...
                  className={`flex items-center justify-center gap-1 rounded-xl border px-2 py-2 text-[11px] font-bold transition disabled:opacity-50 ${
                    status === s.key
                      ? "border-teal-500 bg-teal-50 text-teal-700 dark:border-teal-400 dark:bg-teal-500/15 dark:text-teal-300"
                      : "border-slate-200 bg-white text-slate-600 hover:border-teal-300 dark:border-white/10 dark:bg-[#0c1c27] dark:text-slate-300"
                  }`}
                >
                  {busyStatus === s.key ? <Loader2 size={12} className="animate-spin" /> : s.emoji} {s.short}
                </button>
              ))}
            </div>
          </div>
