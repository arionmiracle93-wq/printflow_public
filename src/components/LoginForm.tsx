...
export function LoginForm() {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      {alasan ? (
        <p className="rounded-xl border border-amber-300/25 bg-amber-400/10 px-3 py-2.5 text-xs font-medium leading-relaxed text-amber-100">
          {alasan}
        </p>
      ) : null}
      <div>
      ...
      <div>
        ...
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-2 text-teal-100/60 transition hover:bg-white/10 hover:text-white"
      ...
      {error ? (
        <p className="rounded-xl border border-rose-400/25 bg-rose-400/10 px-3 py-2 text-xs font-semibold text-rose-200">
          {error}
        </p>
      ) : null}
      <button type="submit" disabled={busy} className="btn-primary w-full">
        <LogIn size={16} /> {busy ? "Memeriksa…" : "Masuk ke Print Flow"}
      </button>
      {needsBootstrap ? (
        <Link
          href="/setup-akun"
          className="flex items-center justify-center gap-1.5 rounded-xl border border-teal-300/25 bg-teal-400/10 px-3 py-2 text-xs font-bold text-teal-100"
        >
          <ShieldCheck size={14} /> Buat akun Owner pertama
        </Link>
      ) : null}
    </form>
  );
