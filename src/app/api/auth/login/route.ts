...
export async function POST(request: Request) {
  ...
  try {
    ...
    await db.insert(loginAudit).values({ username, userId: user?.id ?? null, success: valid, ip, userAgent });
    ...
    await setSessionCookie({ id: user.id, username: user.username, name: user.name, role: user.role as UserRole, tokenVersion: user.tokenVersion, sessionId });
